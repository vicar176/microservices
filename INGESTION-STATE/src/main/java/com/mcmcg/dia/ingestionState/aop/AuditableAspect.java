package com.mcmcg.dia.ingestionState.aop;
...
import com.mcmcg.dia.ingestionState.model.domain.History;
...
public class AuditableAspect {
	...
	public final String SVC_DIA_INGESTION_STATE = "SVC-DIA-INGESTION-STATE";
	...
	private void postStartStopEventToHistory(Auditable auditable) {
		...
		if (started <= 1 && closed <= 1) {
			History history = new History(SVC_DIA_INGESTION_STATE, auditable.eventCode().getCode(), message,
					IngestionStateUtil.formatDate(new Date()));
	...
	private void postEventToHistory(Object returnedValue, Auditable auditable) {
		try{
			...
			History history = new History(SVC_DIA_INGESTION_STATE, code, message, IngestionStateUtil.formatDate(new Date()));
			Response response = restTemplate.postForObject(getHostname() + endpoint, history, Response.class);
	...
	private String getHostname(){
		...
		if (this.hostname == null){
		    ...
		    String scheme = "http";             // http or https
