package com.mcmcg.dia.account.metadata.aop;
...
import com.mcmcg.dia.account.metadata.model.domain.History;
...
public class AuditableAspect {
	...
	public final String MEDIA_SVC_DIA_ACCOUNT_METADATA = "MEDIA-SVC-DIA-ACCOUNT-METADATA";
	...
	private void postStartStopEventToHistory(Auditable auditable) {
		...
		if (started <= 1 && closed <= 1) {
			History history = new History(MEDIA_SVC_DIA_ACCOUNT_METADATA, auditable.eventCode().getCode(), message,
	...
	private void postEventToHistory(Object returnedValue, Auditable auditable) {
		try{
			...
			History history = new History(MEDIA_SVC_DIA_ACCOUNT_METADATA, code, message, MediaMetadataUtil.formatDate(new Date()));
			Response response = restTemplate.postForObject(getHostname() + endpoint, history, Response.class);
