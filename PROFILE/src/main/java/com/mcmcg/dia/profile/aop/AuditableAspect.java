package com.mcmcg.dia.profile.aop;
...
import com.mcmcg.dia.profile.model.domain.History;
...
public class AuditableAspect {
	...
	public final String SVC_DIA_PROFILE = "SVC-DIA-PROFILE";
	...
	private void postStartStopEventToHistory(Auditable auditable) {
		...
		if (started <= 1 && closed <= 1) {
			History history = new History(SVC_DIA_PROFILE, auditable.eventCode().getCode(), message,
					MediaProfileUtil.formatDate(new Date()));
	...
	private void postEventToHistory(Object returnedValue, Auditable auditable) {
		try{
			...
			History history = new History(SVC_DIA_PROFILE, code, message, MediaProfileUtil.formatDate(new Date()));
			Response response = restTemplate.postForObject(getHostname() + endpoint, history, Response.class);
